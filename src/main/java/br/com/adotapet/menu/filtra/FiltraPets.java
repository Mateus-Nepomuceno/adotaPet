...
public class FiltraPets {
    public static void filtraPorTipo(TipoPet tipoPet, List<Pet> listaPets){
        listaPets.removeIf(pet -> !(pet.getTipoPet() == tipoPet));
    }

    public static void filtraPorNomeSobrenome(String nomeSobrenome, List<Pet> listaPets){
        listaPets.removeIf(pet -> !pet.getNomeSobrenome().toLowerCase().contains(nomeSobrenome.toLowerCase()));
    }

    public static void filtraPorSexo(String sexo, List<Pet> listaPets){
        listaPets.removeIf(pet -> !pet.getTipoSexo().SEXO_RELATORIO.equalsIgnoreCase(sexo));
    }

    public static void filtraPorIdade(String idade, List<Pet> listaPets){
        listaPets.removeIf(pet -> !pet.getIdade().toLowerCase().contains(idade.toLowerCase()));
    }

    public static void filtraPorEndereco(String endereco, List<Pet> listaPets){
        listaPets.removeIf(pet -> !pet.getEndereco().toString().contains(endereco));
    }

    public static void filtraPorRaca(String raca, List<Pet> listaPets){
        listaPets.removeIf(pet -> !pet.getRaca().equalsIgnoreCase(raca));
    }

    public static void filtraPorPeso(String peso, List<Pet> listaPets){
